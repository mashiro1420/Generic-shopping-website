<?php
  $san_pham = execute_query("SELECT * FROM san_pham WHERE ma_san_pham = " . $_GET['id'])[0];
  $loai_san_phams = execute_query("SELECT * FROM loai_san_pham WHERE trang_thai=1 ORDER BY ma_loai_san_pham" );
  $nha_san_xuats = execute_query("SELECT * FROM nha_san_xuat WHERE trang_thai=1 ORDER BY ma_nha_san_xuat");
?>
<?php
	$ma_san_pham = $_GET['id'];
	$params = array('ma_san_pham'=>$ma_san_pham);
	$data = execute_query("SELECT * FROM san_pham WHERE ma_san_pham=:ma_san_pham",$params);
	if (count($data) == 0) {
		alert('Mã sản phẩm không tồn tại');
		location('../../quan_ly_san_pham/them_san_pham.php');
		return;
	}
	else
		$san_pham = $data[0];
...
<div class="col-md-5 p-3 d-flex justify-content-center align-items-center">
  <img class="img-thumbnail" src="../../data/san_pham/<?php echo $san_pham['hinh_anh']; ?>" style="object-fit: cover; width: 300px;">
</div>
